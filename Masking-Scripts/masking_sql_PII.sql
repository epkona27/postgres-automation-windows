...
update [dbo].[EMPL_VIEW_CLN1] set 
    MBL_PHN = '(' + STUFF(STUFF(pn.PhoneNumber, 7, 0, '-'), 4, 0, ') ')
FROM
    cte_Tally
...
update  [dbo].[EMPL_VIEW_CLN1] set
    mask_ssn = '' + STUFF(STUFF(pn.ssn, 4, 0, '-'), 7, 0, '-')
FROM
    cte_Tally
    CROSS APPLY ( VALUES (CAST(ABS(CHECKSUM(NEWID())) % 900000000 + 100000001 AS VARCHAR(10))) ) pn (ssn);